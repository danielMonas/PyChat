...
def handleClientInput(msg):
    ...
    commands = {"--help":0,"--promote":2,"--kick":3,"--mute":4,"--unmute":4,"--msg":5,"--viewadmins":6,QUIT:10}
    command = 1
    data = [msg]
    if msg.split(' ',1)[0] in commands.keys():
        ...
        elif msg.split(' ',1)[0] in ["--help","--viewadmins",QUIT]:
            data = ""
        else:
            data = [msg.split(' ',1)[1]]
    send(command,data)
...
class GraphicInterface(wx.Frame):
    ...
    def InitUI(self):
        ...
        vbox.Add(self.msgDisplay,1,  wx.ALL|wx.EXPAND|wx.ALIGN_CENTER_HORIZONTAL, 20)
    ...
    def login(self):
        ...
        while(int(receive()) != 200):
            sname = wx.GetTextFromUser('Please choose a different username', 'Login')
            send("100",[name])
