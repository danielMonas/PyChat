...
def format_msg(msg, name="CONSOLE"):
    """Edits a message to be sent according the chat message format.
        ...
        name = message sender name. Notice the default value, which indicates the message is from the server itself"""
    if name in ADMINS:
        name = '@' + name
...
def get_help(name):
    ...
    if name in ADMINS:
        send(CLIENT_DICT[name], format_msg("--kick [username] = disconnect a user"))
        send(CLIENT_DICT[name], format_msg("--mute [username] = disable a user from sending anything"))
        send(CLIENT_DICT[name], format_msg("--unmute [username] = enable a muted user to send messages"))
        send(CLIENT_DICT[name], format_msg("--promote_user [username] = promote_user a user to Admin rank."))
